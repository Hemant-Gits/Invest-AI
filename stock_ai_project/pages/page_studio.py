...
def _multi_stock_chart(symbols: list[str], period: str):
    ...
    for sym in symbols:
        ...
        fig.add_trace(go.Scatter(x=df["Date"], y=normalized, name=sym, mode="lines"))
...
def render():
    ...
    with config_col:
        ...
        for i, preset in enumerate(PRESET_SYMBOLS[:6]):
            if preset_cols[i % 3].button(preset.split(".")[0], key=f"preset_{preset}"):
        ...
        compare_symbols = st.multiselect("Compare Stocks", PRESET_SYMBOLS, default=[])
    ...
    with main_col:
        ...
        if not analyze and not gen_pdf:
            st.info("Configure inputs and click **Run Full Analysis** to launch the AI prediction engine.")
            return

        with st.spinner("Running XGBoost, sentiment analysis, anomaly detection..."):
            ...
            news = fetch_news(symbol.replace(".NS", ""), 8)
            ...
            st.session_state["studio_results"] = {
                "symbol": symbol, "horizons": horizons, "sentiment": sentiment_agg,
                "vol": vol, "anomaly_status": anomaly_status, "backtest": backtest,
                "comparison": comparison, "explanation": explanation, "change_pct": change_pct,
            }
        ...
        with pulse_tab:
            ...
            c3.metric("Volatility Regime", vol.get("Regime", "N/A"))
            with c4:
        ...
        with risk_tab:
            ...
            if not anomaly_df.empty:
                ...
                fig_anom.add_trace(go.Scatter(
                    x=anomaly_df["Date"], y=anomaly_df["Return"],
                    mode="markers",
                    marker=dict(
                ...
                st.caption(f"Anomalies detected: {detector.count_anomalies(anomaly_df)}")
